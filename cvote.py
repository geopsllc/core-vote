...
async def get_address_data(name, address):
    ...
    if address_data['data']['attributes'].get('vote'):
        voted_delegate = await async_get(api + '/delegates/' + address_data['data']['attributes']['vote'])
        ...
        if voted_rank in range(1, int(active_delegates) + 1) and voted_name in delegate_share_dict:
            reward = round(int(address_balance) / int(voted_balance) * 10800 / int(active_delegates) * block_reward * delegate_share_dict[voted_name] / 100, 3)
        else:
            reward = 0
    else:
        voted_name = 'N/A'
    ...
    for delegate in delegate_share_dict:
        ...
        if delegate != class_dict[name].voted_name and delegate != 'N/A':
            ...
            if new_rank in range(1, int(active_delegates) + 1) and (old_rank <= int(active_delegates) or old_rank_change == 0):
                current_rewards_temp[name] = round(class_dict[name].balance / delegate_votes_temp[delegate] * 10800 / int(active_delegates) * block_reward * delegate_share_dict[delegate] / 100, 3)
                current_ranks_temp[name] = new_rank

                for temp_name in addresses:

                    if temp_name != name and class_dict[temp_name].voted_name == delegate:
                        current_rewards_temp[temp_name] = round(class_dict[temp_name].balance / delegate_votes_temp[delegate] * 10800 / int(active_delegates) * block_reward * delegate_share_dict[delegate] / 100, 3)
                ...
                elif class_dict[name].total < temp_total - 0.01:
...
share_count = share_data['per_page']
...
for i in range(0, delegate_count):
    ...
    delegate_votes_dict[del_name] = int(delegate_data['data'][i]['votes'])
